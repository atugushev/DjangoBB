...
def reputation(request, username):
    ...
    if 'action' in request.GET:
        ...
        viewable_category = Category.objects.viewable(request.user)
        reputations = Reputation.objects.filter(to_user__id=user.id, post__topic__category__id=viewable_category).order_by('-time').select_related()
